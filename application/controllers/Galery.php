...
class Galery extends CI_Controller
{
    ...
    public function s_pemuda()
    {
        ...
        $judul = [
            'title' => 'Management Data',
            'sub_title' => 'Monitoring KP - Mahasiswa',
            'notifikasiBaak' => $this->dashboard->total_rows_baak(),
            'notifikasiKoor' => $this->dashboard->total_rows_koor(),
            'notifikasiSurat' => $this->dashboard->total_rows_surat()
        ];
    ...
    public function pembimbing()
    {
        ...
        $judul = [
            'title' => 'Management Data',
            'sub_title' => 'Monitoring KP - Pembimbing',
            'notifikasiBaak' => $this->dashboard->total_rows_baak(),
            'notifikasiKoor' => $this->dashboard->total_rows_koor(),
            'notifikasiSurat' => $this->dashboard->total_rows_surat()
        ];
        ...
        // var_dump($data);
        $this->load->view('templates/header', $judul);
        $this->load->view('galery/pembimbing',$data);
        $this->load->view('templates/footer');
    }

    public function edit_pembimbing()
    {
        $this->form_validation->set_rules('pembimbing', 'Struktur Pembimbing', 'trim');
        ...
        if ($this->form_validation->run() == false) {
            ...
            // var_dump($data);
            $this->load->view('templates/header', $judul);
            $this->load->view('galery/edit_pembimbing',$data);
            $this->load->view('templates/footer');
        }else{
            $id = $this->uri->segment(3);
            $upload_pemuda = $_FILES['pembimbing']['name'];
            ...
            if ($upload_pemuda) {
                ...
                if ($this->upload->do_upload('pembimbing')) {
                    $old_pemuda = $data['galery']['pembimbing'];
                    unlink(FCPATH . 'assets/galery/' . $old_pemuda);

                    $pembimbing = $this->upload->data('file_name');
                } else {
                    echo $this->upload->display_errors();
                }
            }else {
                $pembimbing = $this->input->post('pembimbing_old');
            }
            // var_dump($pembimbing);
            // die;
            
            $this->galery->UpdatePembimbing($pembimbing,$id);
            $this->session->set_flashdata('success', 'Berhasil Di Update!');
            redirect('galery/pembimbing');
        }    
    }
